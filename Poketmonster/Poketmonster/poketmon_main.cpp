...
void ApearMonster(int mapNum, WMap&map)
{
	...
	{
		PrintMonster(HMonster["불"][singletype].GetFilename(), "불", map);
	}
	else if (5 <= mapNum && mapNum <= 8)
	{
		PrintMonster(HMonster["물"][singletype].GetFilename(), "물", map);
	}
	else if (9 <= mapNum && mapNum <= 12)
	{
		PrintMonster(HMonster["땅"][singletype].GetFilename(), "땅", map);
	}
	else if (13 <= mapNum && mapNum <= 16)
	{
		PrintMonster(HMonster["전기"][singletype].GetFilename(), "전기", map);
	}
